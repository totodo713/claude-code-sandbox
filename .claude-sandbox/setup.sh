...
gather_inputs() {
  ...
  # ---- git push 認証 ----
  # H を選ぶとホストの ~/.gitconfig と ~/.git-credentials を read-only mount する。
  # 認証情報はコンテナ内に永続化せず、ホスト側の credential.helper=store が前提。
  GIT_AUTH_MODE=$(ask_choice "git push 認証 (sandbox 内から push できるようにするか)" \
    "N:無効 (clone/fetch/pull のみ)" \
    "H:ホストの ~/.gitconfig と ~/.git-credentials を read-only mount")
  GIT_USER_NAME=""
  GIT_USER_EMAIL=""
  if [ "$GIT_AUTH_MODE" = "H" ]; then
    [ -f "$HOME/.gitconfig" ] \
      || warn "$HOME/.gitconfig が見つかりません。ホストで git config --global user.name / user.email を設定してください。"
    [ -f "$HOME/.git-credentials" ] \
      || warn "$HOME/.git-credentials が見つかりません。ホストで credential.helper=store を設定し、一度 push して書き出してください (osxkeychain/manager は非対応)。"
    GIT_USER_NAME=$(ask "git commit の author 名 (空 = ホスト .gitconfig の user.name を使用)" "")
    GIT_USER_EMAIL=$(ask "git commit の author email (空 = ホスト .gitconfig の user.email を使用)" "")
  fi
...
write_override() {
  ...
  # Mode H: ホスト ~/.gitconfig と ~/.git-credentials の存在を先にチェックする。
  # 警告は stdout に書かれるので、override.yml 生成ブロックの外で済ませて
  # YAML への混入を防ぐ。マウント対象パスは変数で渡す。
  local git_mount_config="" git_mount_creds=""
  if [ "${GIT_AUTH_MODE:-N}" = "H" ]; then
    if [ -f "$HOME/.gitconfig" ]; then
      git_mount_config="$HOME/.gitconfig"
    else
      warn "git: $HOME/.gitconfig 不在のため mount をスキップ"
    fi
    if [ -f "$HOME/.git-credentials" ]; then
      git_mount_creds="$HOME/.git-credentials"
    else
      warn "git: $HOME/.git-credentials 不在のため mount をスキップ (push 認証は使えません)"
    fi
  fi
...
EOF
    # Mode H で実在を確認できた host file だけマウントする (RO)。
    # コンテナ内に焼かれず、image / volume にも残らない。
    ...
    # GIT_USER_NAME / GIT_USER_EMAIL があれば env で author を上書き
    # (空ならマウントした .gitconfig の user.name/email がそのまま使われる)。
    if [ "${GIT_AUTH_MODE:-N}" = "H" ] && { [ -n "$GIT_USER_NAME" ] || [ -n "$GIT_USER_EMAIL" ]; }; then
      echo "    environment:"
